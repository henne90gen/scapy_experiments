...
int sendEthernetFrame(unsigned short protocolId, void *data, int data_len) {
    const char *iface = "wlp13s0";
    unsigned char dest[ETH_ALEN] = {0x00, 0x12, 0x34, 0x56, 0x78, 0x90};

    int s;
    if ((s = socket(AF_PACKET, SOCK_RAW, htons(protocolId))) < 0) {
    ...
    frame.field.header.h_proto = htons(protocolId);
...
int main(int argc, char **argv) {
    unsigned short protocolId = 0x1337;
    const char *payload = "This is a test package";
    unsigned int payload_length = strlen(payload);
    FPHeader header = {1, payload_length};
    unsigned short header_length = sizeof(header);
    unsigned short data_length = header_length + payload_length;
    void *data = malloc(data_length);
    memcpy(data, &header, header_length);
    memcpy(((char *)data) + header_length, payload, payload_length);

    printf("Sending package\n");
    printf("\theader size: %d\n", header_length);
    printf("\tpayload size: %d\n", payload_length);
    sendEthernetFrame(protocolId, data, data_length);
}
